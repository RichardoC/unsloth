...
# Locks that are NOT the universal, torch-independent kind this file is about.
#
# scripts/gen_macos_bundle_lock.sh produces one single-platform lock for the macOS
# app bundle: one resolution for aarch64-apple-darwin at one Python at one
# deployment target, covering exactly the steps this file's generator documents as
# unlockable (the torch trio, extras.txt with deps, the unsloth/-zoo closure, MLX).
# It is a different artifact with different rules -- it names a torch version on
# purpose, and it re-absorbs the pytorch_tokenizers carve-out precisely because a
# one-platform resolution has no ambiguity to carve out -- so applying this file's
# universal-lock invariants to it would assert the wrong things.
#
# It is not therefore unchecked: tests/security/test_macos_runtime_bundle.py holds it
# to the same and additional properties (every requirement pinned and hashed, no
# ranges, no URL/VCS entries, generated by the pinned uv, one index only), plus the
# platform-specific header it must carry. The membership of this set is itself
# asserted below, so a new lock cannot join it by accident.
PLATFORM_LOCKS: frozenset[str] = frozenset({"darwin-arm64-bundle.lock.txt"})


def _lock_files() -> list[Path]:
    files = sorted(
        path for path in LOCK_ROOT.glob("*.lock.txt") if path.name not in PLATFORM_LOCKS
    )
...
class TestLockContents:
    def test_the_platform_lock_exclusion_names_only_real_platform_locks(self):
        """PLATFORM_LOCKS is an escape from this file's invariants, so it must not be
        possible to land in it by accident. A lock is only allowed out if its own
        header says it was generated by the platform-lock generator AND that it is not
        universal; anything else stays subject to everything below.
        """
        for name in sorted(PLATFORM_LOCKS):
            lock = LOCK_ROOT / name
            assert lock.is_file(), (
                f"PLATFORM_LOCKS names {name}, which does not exist. Remove it from the "
                f"set or regenerate the lock."
            )
            text = lock.read_text(encoding = "utf-8")
            assert "scripts/gen_macos_bundle_lock.sh" in text, name
            assert "NOT universal" in text, name
            assert "unsloth-lock-python-platform:" in text, name
        # And nothing else may claim to be one: a lock naming the platform generator
        # while still inside _lock_files() would be silently held to the wrong rules.
        for lock in _lock_files():
            text = lock.read_text(encoding = "utf-8")
            assert "scripts/gen_macos_bundle_lock.sh" not in text, (
                f"{lock.name} says it was generated by the platform-lock generator but "
                f"is not listed in PLATFORM_LOCKS"
            )
